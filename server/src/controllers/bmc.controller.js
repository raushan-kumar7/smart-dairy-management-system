...
const createBmc = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, createdBMC, "BMC created successfully"));
});
...
const getBMCs = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, bmcs, "BMCs retrieved successfully"));
});
...
const getBMC = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, bmc, "BMC retrieved successfully"));
});
...
const updateBMC = asyncHandler(async (req, res) => {
  ...
  const bmc = await BMC.findOneAndUpdate(
    { bmcCode },
    { $set: updates },
  ...
  if (!bmc) {
    throw new ApiError(500, "Something went wrong while updating BMC");
  }

  return res
    .status(200)
    .json(new ApiResponse(200, bmc, "BMC updated successfully"));
});
...
const deleteBMC = asyncHandler(async (req, res) => {
  ...
  await MPC.findOneAndUpdate({}, { $pull: { bmcs: deletedBMC._id } });

  return res
    .status(200)
    .json(new ApiResponse(200, {}, "BMC deleted successfully"));
});
